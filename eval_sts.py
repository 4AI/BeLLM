...
def main():
    ...
    parser.add_argument('--prompt', type=str, default='The representative word for sentence {text} is:"')
    ...
    model = AnglE.from_pretrained(
        ...
        apply_bfloat16=args.apply_bfloat16).to(device)
    model.set_prompt(prompt=args.prompt)
        ...
        if args.prompt is not None:
            for i, s in enumerate(sentences):
                ...
                if not s.strip():
                    continue
                sentences[i] = {'text': s}
        return model.encode(sentences, device=device, to_numpy=True, max_length=args.max_length)
